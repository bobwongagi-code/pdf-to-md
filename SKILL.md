...
`optimize_file.py` only optimizes image inputs. It does not optimize PDFs. For large PDFs, prefer `--file-url`, or use `split_pdf.py` / the built-in automatic PDF chunking in `vl_caller.py`.

Before the first real parse, prefer a quick configuration check:
...
**Operational defaults for stability and speed**:
   - `vl_caller.py` now validates local file existence before calling the API
   - Successful local-file parses are cached under the system temp directory
   - Re-running the same local file can reuse the cached JSON instead of calling the API again
   - Network/API failures use bounded retries with backoff
   - Local PDFs over 100 pages are automatically split into stable chunks and merged back into one result
   - Large local PDFs also benefit from chunk-level cache reuse during repeat runs
   - Use `--no-cache` only when you explicitly need a fresh API response
   ...
   **Optional preprocessing flags**:
   ```bash
   python scripts/vl_caller.py --file-path "scan.pdf" --pretty --doc-unwarping --orientation-classify
   ```
   - `--doc-unwarping`: helps with warped or curved scanned pages
   - `--orientation-classify`: helps with rotated pages
...
For local PDFs, `vl_caller.py` automatically chunks files larger than 100 pages into multiple requests and merges the results. This is the preferred stable path for oversized PDFs.
...
#### Repeated Local Runs
If you expect to parse the same local file multiple times while iterating, keep using `--file-path` and let the built-in cache absorb repeat runs:
```bash
python scripts/vl_caller.py --file-path "./financial_report.pdf" --pretty
```

If you need to bypass cache for a fresh response:
```bash
python scripts/vl_caller.py --file-path "./financial_report.pdf" --pretty --no-cache
```
